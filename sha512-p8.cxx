...
void sha512_process_p8(uint64_t state[8], const uint8_t data[], uint32_t length)
{
    ...
    const uint64_t* k = reinterpret_cast<const uint64_t*>(K);
    ...
    while (blocks--)
    {
        ...
        k = reinterpret_cast<const uint64_t*>(K);
        unsigned int i, offset=0;

        // We have to unroll the loop to get the constexpr of the round number
